...
public class ChainHash<K,V> {
 ...
 class Node<K,V> { //내부 클래스
     private K key;                 // 키값
     private V data;                // 데이터
     private Node<K,V> next;        // 뒤쪽 포인터(뒤쪽 노드에 대한 참조).    링크드리스트 있으니까.
 ...
 //--- 생성자(constructor) ---// 테이블만 만들고 각 노드는 add 할 때 만듦
 ...
 public int add(K key, V data) {
     ...
     Node<K,V> p = table[hash];            // 선택 노드. 포인터
     ...
     Node<K,V> temp = new Node<K,V>(key, data, table[hash]); //새로운 노드 만들어서 삽입
